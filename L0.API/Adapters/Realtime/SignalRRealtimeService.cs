...
public class SignalRRealtimeService(
  ...
  private static readonly ConcurrentDictionary<Type, IHubContext> HubContextCache = new();
  ...
  public async Task PublishAsync(string hubKey, string group, string method, object data,
    ...
    var hubContext = HubContextCache.GetOrAdd(hubType, type => {
      ...
      return (IHubContext)serviceProvider.GetRequiredService(hubContextType);
    });

    await hubContext.Clients.Group(group).SendAsync(method, data, ct);
  }
}
