...
public class RevokeItemHandlerTests {
  ...
  private readonly RevokeItemHandler _sut;

  public RevokeItemHandlerTests() {
    _sut = new RevokeItemHandler(_repository);
  }

  [Fact]
  public async Task Handle_Should_ThrowWorkflowException_When_ItemMissingOrNotOwned() {
    var request = new RevokeItemCommand(Guid.NewGuid(), Guid.NewGuid());
  ...
  [Fact]
  public async Task Handle_Should_RevokeItem_And_ReturnTrue() {
    var item = new CatalogItemBuilder().WithOwnerId(Guid.NewGuid()).Build();
    var request = new RevokeItemCommand(item.Id, item.OwnerId);
