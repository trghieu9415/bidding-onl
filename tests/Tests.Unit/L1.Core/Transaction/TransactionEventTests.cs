...
public class TransactionEventTests {
  ...
  public void Events_ExposeExpectedAggregateIds() {
    ...
    var completedEvent = new OrderCompletedEvent(orderId, Guid.NewGuid(), Guid.NewGuid(), "Bidder", "bidder@example.com");
    var createdEvent = new OrderCreatedEvent(orderId, Guid.NewGuid(), Guid.NewGuid());
    var paymentCompletedEvent = new PaymentCompletedEvent(paymentId, Guid.NewGuid(), 150m, PaymentMethod.Paypal, "txn-001");
    var paymentRefundedEvent = new PaymentRefundedEvent(paymentId, Guid.NewGuid(), 150m, PaymentMethod.Paypal, "txn-001");
